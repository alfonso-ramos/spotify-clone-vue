...
<template>
    <li class="flex items-center justify-between rounded-md hover:bg-[#2A2A2A]" @mouseenter="isHover = true"  @mouseleave="isHover = false"
    >
        <div class="flex items-center w-full py-1.5">
            <div v-if="isHover" class="w-[40px] ml-[14px] mr-[6px] cursor-pointer">
                <Play v-if="true" fillColor="#fff" :size="25"/>
            ...
            <div>
                ...
                <div class="text-sm fonst-semibold text-gray-400">{{artist.name}}</div>
            </div>
        </div>
